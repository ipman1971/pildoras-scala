...
class UserRepositoryTest extends FunSuite {
  ...
  test("probando el metodo getOrElse cuando existe el valor") {
    val usuario: Usuario = UserRepository.findById(1).get
    assert(usuario.genero.getOrElse("no especificado") == "hombre")
  }

  test("probando el metodo getOrElse cuando no existe el valor") {
    val usuario: Usuario = UserRepository.findById(2).get
    assert(usuario.genero.getOrElse("no especificado") == "no especificado")
  }
